...
- 安装：`install-skill.sh <skill 目录>`（软链到 `~/.claude/skills`、`~/.codex/skills`、`~/.config/opencode/skills`、`~/.agents/skills` 四个客户端目录）；卸载：`uninstall-skill.sh <skill 名或路径>`
...
- 四个安装目标里 `~/.agents/skills` 是跨工具标准路径，ZCode 等 runtime 从这里读 skill；只装另三个会导致 skill 在那些客户端里不可见
